...
#define MAX(x, y) (x > y ? x : y)
    ...
    if (temp_M % 384 == 0 && temp_N % 64 == 0 && temp_K % 64 == 0)
    {
        //No padding
        for (blk_M = 64; blk_M <= (temp_M / 6); blk_M += 64)
        {
            for (blk_N = 64; blk_N <= temp_N; blk_N += 64)
            {
                for (blk_K = 64; blk_K <= temp_K; blk_K += 64)
                {
                    ldm_use = sizeof(float) * (5 * blk_N * blk_K + 4 * blk_K * blk_M + 3 * blk_N * blk_M) / 64;
                    if (ldm_use < 215 * 1024 && (temp_M / 6) % blk_M == 0 && temp_N % blk_N == 0 && temp_K % blk_K == 0)
                    {
                        temp_blk_M = blk_M;
                        temp_blk_N = blk_N;
                        temp_blk_K = blk_K;
    ...
    } //&& (((temp_K +blk_K -1) / blk_K) % 6 == 0 || ((temp_K +blk_K -1) / blk_K) % 6 >= 4)
    else if (ldm_use < 215 * 1024 && temp_N % 64 == 0 && temp_K % 64 == 0)
    {
        blk_N = temp_N;
        blk_K = temp_K;
        ...
        {
            ldm_use = sizeof(float) * (5 * blk_N * blk_K + 4 * blk_K * blk_M + 3 * blk_N * blk_M) / 64;
            if (ldm_use < 215 * 1024 && (((temp_M + blk_M - 1) / blk_M) % 6 == 0 || ((temp_M + blk_M - 1) / blk_M) % 6 >= 5))
            {
                int bsizeN = blk_N / 8 * sizeof(float);
                int bsizeM = blk_M / 8 * sizeof(float);

                double T_dma = temp_N / blk_N * (temp_M / 6) / blk_M * temp_K / blk_K * (1.0 * blk_N * blk_K * sizeof(float) / 1e6 / MBW_map[bsizeN / 16 - 1] + 1.0 * blk_M * blk_K * sizeof(float) / 1e6 / MBW_map[bsizeN / 16 - 1]) +
                               1.0 * temp_N / blk_N * (temp_M / 6) / blk_M * blk_M * blk_N * sizeof(float) / 1e6 / MBW_map[bsizeN / 16 - 1];
                ...
                float T_compute = estimite_compute_time(blk_M, blk_N, blk_K, (temp_M / 6), temp_N, temp_K);
    ...
    else if (temp_N % 64 == 0 || temp_K % 64 == 0)
    {
        for (blk_M = 64; blk_M <= temp_M && blk_M <= 8192; blk_M += 64)
        {
            for (blk_N = 64; blk_N <= temp_N && blk_N <= 8192; blk_N += 64)
            {
                ...
                {
                    ldm_use = sizeof(float) * (5 * blk_N * blk_K + 4 * blk_K * blk_M + 3 * blk_N * blk_M) / 64;
                    if (ldm_use < 215 * 1024 && (temp_N % blk_N == 0 || temp_K % blk_K == 0) && (((temp_M + blk_M - 1) / blk_M) % 6 == 0 || ((temp_M + blk_M - 1) / blk_M) % 6 >= 5))
                    {
                        int bsizeN = blk_N / 8 * sizeof(float);
                        int bsizeM = blk_M / 8 * sizeof(float);
...
#define MAX(x, y) (x > y ? x : y)
    ...
    {
        //No padding
        for (blk_M = 64; blk_M <= (temp_M + blk_M / 6); blk_M += 64)
        {
            for (blk_N = 64; blk_N <= temp_N; blk_N += 64)
            {
                for (blk_K = 64; blk_K <= temp_K; blk_K += 64)
...
#define MAX(x, y) (x > y ? x : y)
    ...
    if (temp_M % 64 == 0 && temp_N % 64 == 0 && temp_K % 384 == 0)
    {
        //No padding
        for (blk_M = 64; blk_M <= temp_M; blk_M += 64)
        {
            for (blk_N = 64; blk_N <= temp_N; blk_N += 64)
            {
                for (blk_K = 64; blk_K <= (temp_K + blk_K) / 6; blk_K += 64)
                {
                    ldm_use = sizeof(float) * (5 * blk_N * blk_K + 4 * blk_K * blk_M + 3 * blk_N * blk_M) / 64;
                    if (ldm_use < 215 * 1024 && temp_M % blk_M == 0 && temp_N % blk_N == 0 && (temp_K / 6) % blk_K == 0)
                    {
                        temp_blk_M = blk_M;
                        temp_blk_N = blk_N;
                        temp_blk_K = blk_K;
    ...
    else if (ldm_use < 215 * 1024 && temp_M % 64 == 0 && temp_N % 64 == 0)
    {
        //printf("crr blk condition 1\n");
        blk_M = temp_M;
        blk_N = temp_N; //(temp_K + blk_K / 6)
        for (blk_K = 64; blk_K <= (temp_K + blk_K / 6); blk_K += 64)
        {

            ldm_use = sizeof(float) * (4 * blk_N * blk_K + 5 * blk_K * blk_M + 3 * blk_N * blk_M) / 64;

            if (ldm_use < 215 * 1024 && (((temp_K + blk_K - 1) / blk_K) % 6 == 0 || ((temp_K + blk_K - 1) / blk_K) % 6 >= 5))
            {
    ...
    {
        //printf("crr blk condition 2\n");
        for (blk_M = 64; blk_M <= temp_M && blk_M <= 4096; blk_M += 64)
        {
            for (blk_N = 64; blk_N <= temp_N && blk_N <= 4096; blk_N += 64)
            {
                for (blk_K = 64; blk_K <= (temp_K + blk_K / 6); blk_K += 64)
                {
                    ...
                    if (ldm_use < 215 * 1024 && (((temp_K + blk_K - 1) / blk_K) % 6 == 0 || ((temp_K + blk_K - 1) / blk_K) % 6 >= 5))
                    {
                        ...
                        double T_dma = temp_N / blk_N * temp_M / blk_M * (temp_K / 6) / blk_K * (1.0 * blk_N * blk_K * sizeof(float) / 1e6 / MBW_map[bsizeN / 16 - 1] + 1.0 * blk_M * blk_K * sizeof(float) / 1e6 / MBW_map[bsizeN / 16 - 1]) +
                                       1.0 * temp_N / blk_N * temp_M / blk_M * blk_M * blk_N * sizeof(float) / 1e6 / MBW_map[bsizeN / 16 - 1];

                        double T_init_dma = (1.0 * blk_N * blk_K * sizeof(float) / 1e6 / MBW_map[bsizeN / 16 - 1] +
                                             1.0 * blk_M * blk_K * sizeof(float) / 1e6 / MBW_map[bsizeM / 16 - 1]);

                        float T_compute = estimite_compute_time(blk_M, blk_N, blk_K, temp_M, temp_N, (temp_K / 6));
